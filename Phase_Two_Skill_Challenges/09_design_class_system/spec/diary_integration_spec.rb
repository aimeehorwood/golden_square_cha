...
describe "diary reading behaviour" do
  context "when there is a perfect diary entry to read in the time" do
  ...
  context "when there is nothing readable in the time" do
    it "returns nil" do
      diary = Diary.new
      reader = DiaryReader.new(2, diary)
 ...
 describe "phone number extraction behaviour" do
    ...
    it "doesnt extract invalid numbers" do
        diary = Diary.new
        phone_book = PhoneNumberCrawler.new(diary)
        diary.add(DiaryEntry.new("title", "my friend 078000000 is cool"))
        expect(phone_book.extract_numbers).to eq []
    end
  end
end
